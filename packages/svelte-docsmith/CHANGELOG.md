...
- b01d6b0: - Scope the code block styles to the component's own `<pre>`, so they no longer restyle unrelated code blocks in the consuming app
...
- 4ba974f: - Date every page from one git history walk instead of a `git log` per page, cutting the content index's git cost by about 11x (93ms to 8.5ms on a 23-page site) and scaling with history rather than page count
...
- 5e0f2ba: Give the `archive-version` command one seam into the library, so version identity and commit dates are decided in one place instead of two.
...
- a922f2e: - The mobile menu now shows the version switcher on non-doc pages (a landing page, or any `layout="page"` route), matching what the desktop header already did there.
