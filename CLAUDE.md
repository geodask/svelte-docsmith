...
- `src/lib/buildtime/` — Node-only build tooling. `preprocess.ts` (the `svelte-docsmith/preprocess` mdsvex + Shiki preprocessor), `vite/` (the `svelte-docsmith/vite` plugin, split into `pages.ts`/`frontmatter.ts`/`extract.ts`/`collect.ts`/`releases.ts` collaborators behind `vite/index.ts`), `git.ts` (every git query the build makes, shared by the plugin and the CLI, which is why it sits beside `vite/` rather than inside it), `archive.ts`/`archives.ts` and `cli/` (the `archive-version` command), `paths.ts` (routes-relative directory to URL), `highlight.ts` (shared Shiki config), and `markdown-layout.svelte` (the injected mdsvex default layout). Named `buildtime`, not `build`, because a `build/` dir is gitignored.
- `src/lib/generate/` — framework-agnostic string generators wired into routes: `sitemap.ts`, `llms.ts` (`generateLlmsTxt`/`generateLlmsFullTxt`), `feed.ts` (the changelog's Atom feed), and `xml.ts` (the escaping the two XML generators share).
...
- `src/lib/utils/` — small runtime helpers. `cn.ts` is the shadcn `cn()` plus its bits-ui type helpers (the `utils` alias in `components.json` points here). `url.ts` is the one vocabulary every layer builds doc URLs with (`normalizePath`, `join`, `under`, `firstSegmentUnder`) — reach for it rather than writing a slash regex inline. Also `clipboard.svelte.ts`, `types.ts`.
