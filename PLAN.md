...
- [x] Public API contract refactor (2026-07-04) — froze the minimal public surface (see below); dead-code purge, test bootstrap, velite removed from the consumer path (`svelte-docsmith/content` + a content-index `docsmith()` vite plugin), `index.ts` trimmed, broken breadcrumb feature deleted, internal TOC engine simplified, `docs-shell` decoupled into parts, ESLint wired into `lint`, `npm pack` tarball artifact added to CI

**Public API contract (frozen 2026-07-04 — supersedes §6.2 tiers 2–3).** The owner's directive: agree a minimal public surface, everything else is internal and freely changeable. A consumer touches exactly five things — `svelte.config.js` (the `docsmith()` preprocess call), `vite.config.ts` (the `docsmith()` plugin: content index + `?source` transform), `app.css` (`@import 'svelte-docsmith/theme.css'`), `+page.md` files with frontmatter, and one `DocsShell` layout. The entire export map:
- `svelte-docsmith` → `DocsShell`, `LiveExample`, `Tabs`, `TabItem`, `defineConfig`, `type DocsmithConfig`, `type DocsContentItem`
- `svelte-docsmith/preprocess` → `docsmith(options)` (mdsvex pipeline)
- `svelte-docsmith/vite` → `docsmith(options)` (content index + `?source`)
- `svelte-docsmith/content` → `docs: DocsContentItem[]` (generated from frontmatter by the vite plugin)
- `svelte-docsmith/theme.css`

Everything else is internal: the TOC engine (`reactiveToc`, `tocFromContent`, `visibilityObserver`, and their types), the shadcn primitives, the markdown component map, `useClipboard`, `BackgroundPattern`. `DocPage`/`TableOfContents`/`navFromContent`/breadcrumb/`WithChildren` and the `exampleSource` alias were removed. Pre-1.0, no external adopters — no deprecation cycle. A "customizer tier" can be re-added deliberately later if a real need appears.

**Watch item — SvelteKit config-less migration (not adopted now).** Kit 3 folds `svelte.config.js` into `vite.config.js` (§2.4). The contract can't drop `svelte.config.js` today: `svelte-package` still requires it to build the library, and the docs site must dogfood the exact README adoption path. When Kit 3 ships stable, move the docs site's config into `sveltekit({...})` inside `vite.config.ts` and rewrite the README's two-file story in the *same* commit — it's a docs change, not an API change, because `docsmith()` is registration-location-agnostic by design.
...
- `pnpm lint` runs Prettier **and** ESLint (wired 2026-07-04). The flat config's Svelte-file block now sets `parserOptions.parser: ts.parser` so `<script lang="ts">` parses; both workspaces are clean. ESLint relies on the `.svelte-kit` types generated by the `prepare` (`svelte-kit sync`) script, which `pnpm install` runs — so lint works on a fresh checkout.
